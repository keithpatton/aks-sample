...
resource "azurerm_mssql_firewall_rule" "aks" {
  ...
  server_id           = azurerm_mssql_server.default.id
  start_ip_address    = "${split(",",azurerm_kubernetes_cluster.default.network_profile.pod_cidr)[0]}"
  end_ip_address      = "${split(",",azurerm_kubernetes_cluster.default.network_profile.pod_cidr)[1]}"
